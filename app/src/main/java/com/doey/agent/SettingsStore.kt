...
class SettingsStore(private val context: Context) {
    ...
    // ── DataStore flows ───────────────────────────────────────────────────────
    val provider: Flow<String>       = context.dataStore.data.map { it[KEY_PROVIDER] ?: "groq" }
    val model: Flow<String>          = context.dataStore.data.map { it[KEY_MODEL] ?: "llama-3.3-70b-versatile" }
