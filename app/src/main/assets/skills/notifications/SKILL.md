...
The `instruction` is the command the sub-agent will execute when a matching notification arrives. Write it as if you were giving Doey the command directly. The sub-agent has access to all tools (TTS, WhatsApp, SMS, Gmail, etc.).
...
**Important**: The user must manually enable "Notification access" for Doey in the Android settings.
